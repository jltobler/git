 ...
 *
 * All missing files (across all packs) are collected into a single
 * s3_storage_download_files() call so that curl_multi can drive them concurrently.
 * Activation (packfile_store_load_pack) happens only after every download
 * has completed, since it requires the .idx to be on disk.
 */
static void odb_source_s3_prepare(struct odb_source_s3 *s3)
{
	static const char * const exts[] = { "pack", "rev", "idx" };
	...
	for (size_t i = 0; i < manifest->packs.nr; i++) {
		...
		for (size_t j = 0; j < ARRAY_SIZE(exts); j++) {
			strbuf_reset(&path);
			strbuf_addf(&path, "%s/packs/%s.%s",
				    s3->storage->cache_dir, hash, exts[j]);
...
static void odb_source_s3_reprepare(struct odb_source *source)
{
	...
	/*
	 * Drop the source-level initialized flag so that the next access
	 * re-runs odb_source_s3_prepare(). Note that the underlying
	 * s3_storage caches the resolved manifest for the lifetime of the
	 * process, so reprepare() will not pull a fresh manifest from S3;
	 * it only re-runs the local steps (re-checking the cache and
	 * registering any packs that were not yet loaded).
	 */
...
/*
 * Append the .pack, .idx and .rev files of a single pending pack to the
 * caller-provided s3_transfers list. The caller passes the resulting entries
 * to s3_storage_upload_files(); putting multiple packs into the same call lets
 * curl_multi drive them concurrently.
 *
 * Sets pending->pack_basename on success so that callers can construct
 * the manifest entry after the upload completes.
 */
static void s3_append_pending_pack(struct s3_transfers *transfers,
				   struct s3_pending_pack *pending)
{
	s3_transfers_append(transfers, pending->pack_path);
	s3_transfers_append(transfers, pending->idx_path);
	s3_transfers_append(transfers, pending->rev_path);

	pending->pack_basename = strrchr(pending->pack_path, '/') + 1;
}
			 ...
			 size_t objects_nr)
{
	...
	/*
	 * Collect the .pack/.idx/.rev plus the new manifest body into a single
	 * s3_storage_upload_files() call so they upload concurrently.
	 */
	s3_append_pending_pack(&transfers, &pending_pack);
...
static int odb_transaction_s3_commit(struct odb_transaction *base)
{
	...
	struct s3_transfers transfers = S3_TRANSFERS_INIT(tx->s3->storage);
	struct s3_manifest new_manifest = S3_MANIFEST_INIT;
	...
	/*
	 * Collect every pack's three artefacts plus the new manifest body into
	 * a single s3_storage_upload_files() call. We need each pack's
	 * basename for the manifest `p:` lines, which is set by
	 * s3_append_pending_pack().
	 */
	for (size_t i = 0; i < tx->packs_nr; i++)
		s3_append_pending_pack(&transfers, &tx->packs[i]);

	s3_manifest_copy(s3_storage_get_manifest(tx->s3->storage), &new_manifest);
	for (size_t i = 0; i < tx->packs_nr; i++) {
	...
	if (s3_transfers_publish(tx->s3->storage, &new_manifest, &transfers)) {
		ret = -1;
		goto out;
	}
					 ...
					 struct odb_transaction_write_pack_opts *opts)
{
	 ...
	 * against the same cache directory do not clobber each other. We
	 * immediately delete the tempfile after creation so that
	 * index-pack can recreate it with O_CREAT|O_EXCL; the .idx and
	 * .rev paths are derived from the same unique base.
	 */
	strbuf_addf(&tmp_pack_template, "%s/packs/tmp_pack_XXXXXX.pack",
		    tx->s3->storage->cache_dir);
	tmp_pack_tempfile = mks_tempfile_s(tmp_pack_template.buf, strlen(".pack"));
	if (!tmp_pack_tempfile) {
		ret = error_errno("s3: cannot create tempfile under '%s'",
				  tx->s3->storage->cache_dir);
		goto out;
	}
...
static const char **odb_transaction_s3_env(struct odb_transaction *base)
{
	...
	s3_storage_write_manifest(tx->s3->storage, &tx->manifest, &path);
				  ...
				  const struct odb_optimize_options *opts)
{
	...
	/*
	 * Compose the new manifest first so we can fold its body upload
	 * into the same call as the merged pack files.
	 *
	...
	/*
	 * Upload .pack, .idx, .rev and the new manifest body concurrently in a
	 * single s3_storage_upload_files() call. The pointer PUT must come
	 * after the upload returns so that we do not advance to a body that
	 * hasn't been uploaded yet.
	 */
	s3_transfers_append(&transfers, pack_path);
	s3_transfers_append(&transfers, idx_path);
	s3_transfers_append(&transfers, rev_path);

	if (s3_transfers_publish(s3->storage, &new_manifest, &transfers) < 0) {
		ret = -1;
		goto out;
	}
