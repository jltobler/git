...
test_expect_success 'pruned caches are re-downloaded' '
	...
	(
		...
		ls .git/s3-cache/reftables/*.ref >tables-after &&
...
test_expect_success 'incremental pushes append refs and packs to the manifest' '
	...
	(
		...
		env GIT_S3_MANIFEST="$MANIFEST_FIRST" git -C ../target.git refs list >actual &&
		test_cmp expect-first actual &&
		env GIT_S3_MANIFEST="$MANIFEST_SECOND" git -C ../target.git refs list >actual &&
